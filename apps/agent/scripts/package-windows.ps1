...
$playwrightCore = (Get-ChildItem "$root\node_modules\.pnpm" -Directory -Filter 'playwright-core*' | Select-Object -First 1).FullName
Copy-Item "$playwrightCore\node_modules\playwright-core" "$Output\node_modules\playwright-core" -Recurse -Force
...
@('@echo off', 'set "PLAYWRIGHT_BROWSERS_PATH=%~dp0playwright"', 'node --experimental-specifier-resolution=node "%~dp0dist\src\cli\main.js" %*') | Set-Content "$Output\lazybot-agent.cmd" -Encoding ASCII
